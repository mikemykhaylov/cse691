...
class LookaheadAgent:
    ...
    def __init__(self,
                 player_id: int,
                 lookahead_depth: int,
                 env_for_copying: SuperTicTacToe3DEnv, # Pass a sample env instance
    ...
    def _minimax_search(self, env_state: SuperTicTacToe3DEnv, depth: int) -> float:
        ...
        for action in valid_actions:
            ...
            # Optional: Alpha-Beta Pruning could be added here for efficiency
            # but requires passing alpha and beta values down the recursion.
    ...
    def choose_action(self, observation: Dict[str, Any], info: Dict[str, Any]) -> int:
        ...
        for action in valid_actions:
            ...
            initial_env_copy._is_terminal, initial_env_copy._game_winner = self._check_terminal_status(initial_env_copy) # Helper needed
...
if __name__ == '__main__':
    ...
    try:
        player1_agent = LookaheadAgent(player_id=PLAYER1,
                                       lookahead_depth=LOOKAHEAD_DEPTH,
                                       env_for_copying=env_for_logic,
                                       rng=rng)
    ...
    while not terminated and not truncated and step_count < max_steps:
        ...
        render_obs, render_reward, render_terminated, render_truncated, render_info = render_env.step(action) # Render env also steps
    ...
    if winner == PLAYER1: print(f"Winner: Player 1 (X) - Lookahead(n={LOOKAHEAD_DEPTH})")
    elif winner == PLAYER2: print("Winner: Player 2 (O) - Heuristic")
    elif winner == DRAW: print("Result: Draw")
    else: print("Result: Unknown or Ongoing (check max_steps)")
